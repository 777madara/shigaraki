...
const gameSlot = async (conn, m, db) => {
	...
	try {
		if (slot1 === slot2 && slot2 === slot3) {
			...
			let sloth =`[  🎰VIRTUAL SLOT 🎰  ]\n------------------------\n\n${listSlot1}\n${listSlot2} <=====\n${listSlot3}\n\n------------------------\n[  🎰 VIRTUAL SLOT 🎰  ]\n\n*معلومة* :\n_لقد ربحت🎉_ <=====Limit + ${randomLimit}, Uang + ${randomLimit * 500}`
...
const gameCasinoSolo = async (conn, m, prefix, db) => {
	try {
		...
		if (m.args.length < 1) return m.reply(prefix + 'casino <الكمية>\n' + prefix + 'casino 1000')
		if (isNaN(m.args[0])) return m.reply(`! ادخل المبلغ\nمثال : ${prefix + m.command} 1000`)
		if (db.users[m.sender].money >= count * 1) {
			db.users[m.sender].limit -= 1
			db.users[m.sender].money -= count * 1
			db.set[botNumber].money += count * 1
			if (Aku > Kamu) {
				m.reply(`💰 Casino 💰\n*Kamu:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*You LOSE*\nدولار ${count} لقد خسرت`.trim())
			} else if (Aku < Kamu) {
				db.users[m.sender].money += count * 2
				m.reply(`💰 Casino 💰\n*Kamu:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*You Win*\nدولار ${count * 2} لقد ربحت`.trim())
			} else {
				db.users[m.sender].money += count * 1
				m.reply(`💰 Casino 💰\n*Kamu:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*SERI*\nدولارn ${count * 1} لقد حصلت على`.trim())
			}
		} else m.reply(`n *kumpulkan*!اموالك لا تكفي للكازينو يرجى التحصيل اولا `)
	} catch (e) {
		m.reply('Error!')
	}
}
...
const gameSamgongSolo = async (conn, m, db) => {
	...
	if (isNaN(count) || count < 5000) return m.reply('! الحد الادنى للرهان هو 5000');
	if (db.users[m.sender].money < count) return m.reply(` *kumpulkan* !اموالك لا تكفي يرجى التحصيل اولا`)
	db.users[m.sender].money -= count;
	db.users[m.sender].limit -= 1
	let { key } = await m.reply('...بدات اللعبة !* يتم توزيع البطاقات🃏*');
	await sleep(5000);
